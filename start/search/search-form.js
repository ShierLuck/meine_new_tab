(function(e) {
    ...
    function a() {
        ...
        trackStatusEvent("search-" + t.ShortName, null, a, function() {
            try {
                var t = [];
                if (localStorage.getItem("se_txt"))
                    t = ("" + localStorage.getItem("se_txt")).split("|");
                if (t.indexOf(a) < 0) {
                    if (t.length >= 50)
                        t.shift();
                    t.push(a);
    ...
    $(document).ready(function() {
        ...
        if (localStorage.getItem("hideLink"))
            c = JSON.parse(localStorage.getItem("hideLink"));
        var u = [];
                ...
                $.getJSON(a, function(a) {
                    try {
                        if (a && a.query && a.query.count == 1 && a.query.results && a.query.results.channel && a.query.results.channel.item) {
                            ...
                            if (localStorage.getItem("disable_weather") == "yes" || y)
                                b = false;
                            else
                                b = true
                        } else {
                    ...
                    utils.localstorage2cookie();
                    if (e.debug)
                        console.log("Error in weather request: ", o)
                })
            } catch (e) {
                y = true;
                b = false;
        ...
        function G(o) {
            ...
            try {
                if (e.autoSuggest != null)
                    e.autoSuggest.setSuggestUrl(a["SuggestUrl"])
            } catch (e) {}
            utils.localstorage2cookie();
        ...
        chrome.runtime.onMessage.addListener(function(t, o, a) {
            ...
            if (t.showNotifyDialog) {
                ...
                var l = 0;
                if (localStorage.getItem("LNT-" + r.name) && !isNaN(parseInt(localStorage.getItem("LNT-" + r.name))))
                    l = parseInt(localStorage.getItem("LNT-" + r.name));
