(function (e) {
  ...
  localStorage.setItem(
    "newtab_url",
    chrome.extension.getURL("/start/index.html")
  );
  ...
  chrome.browserAction.onClicked.addListener(function () {
    ...
    chrome.tabs.create({ url: localStorage.getItem("newtab_url") });
  });
  chrome.runtime.setUninstallURL(
    user["firstRunLandingPage"] + "?ext_uninstall&id=" + chrome.runtime.id
  );
  var a = utils.get;
  var o = utils.set;
  ...
  function l() {
    ...
    var t =
      ...
      "&idt=" +
      localStorage.getItem("installdt") +
      "&dsb=" +
      localStorage.getItem("installdc") +
      "&grp=" +
      localStorage.getItem("user_group") +
      "&ver=" +
      localStorage.getItem("version") +
      "&gmh=" +
      localStorage.getItem("gmh") +
      "&eid=" +
      chrome.runtime.id;
          ...
          if (e.relate && e.relate.length) {
            chrome.tabs.query({}, function (e) {
              for (var t = 0; t < e.length; t++) {
                chrome.tabs.sendMessage(e[t].id, { refreshRelativeApps: true });
              }
            });
          }
        }
    ...
    chrome.tabs.query({}, function (e) {
      for (var t = 0; t < e.length; t++) {
        chrome.tabs.sendMessage(e[t].id, {
          ...
          info: { error_msg: "Unable to get your city." }
        });
      }
    });
  }
  function c(t) {
    ...
    $.getJSON(a, function (a) {
      ...
      if (r) {
        var l = { enteredLocation: t, woeid: n.woeid, location_name: n.city };
        localStorage.setItem("weather_location", JSON.stringify(l));
        chrome.tabs.query({}, function (e) {
          for (var t = 0; t < e.length; t++) {
            chrome.tabs.sendMessage(e[t].id, { refreshWeather: true });
          }
        });
      } else {
  ...
  chrome.runtime.onMessage.addListener(function (t, a, o) {
    ...
    if (t.ext) {
      ...
      o({ ext: JSON.stringify(localStorage) });
    } else if (t.topSites) {
    ...
    if (t.type === "weather_location_request") {
      ...
      if (e.debug)
        console.log("request.info.enteredLocation", t.info.enteredLocation);
        ...
        for (var n = 0; n < e.length; n++) {
          if (e[n].id !== a.tab.id) {
            chrome.tabs.sendMessage(e[n].id, {
              updateNote: { noteChange: t.noteChange }
            });
            o({ updateSent: true });
          }
        }
    ...
    $.post(
      ...
      function (t) {
        ...
        if (t && t.wl && t.wl.length) {
          ...
          var n = utils.getAppsInList2ThatNotInList1(
            [].concat([{ id: chrome.runtime.id }], o.relate),
            t.wl
          );
          ...
          if (a === "onInstalled" || a === "onEnabled") {
            ...
            setTimeout(function () {
              chrome.runtime.sendMessage(
                ...
                function (t) {
                  if (e.debug)
                    console.log("sync " + chrome.runtime.id + " - " + t);
                }
              );
  ...
  chrome.tabs.onCreated.addListener(function (t) {
    if (t.url.match("chrome://newtab/")) {
      ...
      if (e.debug)
        console.log("last open was " + Math.floor((a - o) / 1e3) + "s ago");
      if (a - o > n * 6e4) {
      ...
      if (localStorage.getItem("random_all_newtab") == "yes") {
        ...
        if (s.length > 0) {
          utils.getEnabledAppsInWhitelist(s, function (e) {
            ...
            chrome.tabs.update(t.id, { url: o }, function (e) {});
          });
        }
      }
    }
  ...
  function p(t) {
    ...
    chrome.cookies.get(
      { url: "http://" + user["firstRunDomain"] + "/", name: "CKT-" + t.name },
      function (e) {
        var a = 0;
        ...
        chrome.cookies.set({
          ...
          value: a + 1,
          expirationDate:
            Math.floor(new Date().getTime() / 1e3) + 30 * 24 * 60 * 60
        });
      }
    );
    var a = 0;
    if (
      localStorage.getItem("LNT-" + t.name) &&
      !isNaN(parseInt(localStorage.getItem("LNT-" + t.name)))
    )
      a = parseInt(localStorage.getItem("LNT-" + t.name));
    localStorage.setItem("LNT-" + t.name, a + 1);
    chrome.notifications.create(chrome.runtime.id + t.name, t.noti, function (
      a
    ) {
      chrome.notifications.onClicked.addListener(function (o) {
        if (o == a) {
          ...
          if (t["lp0"]) chrome.tabs.create({ url: t["lp0"] });
      ...
      chrome.notifications.onButtonClicked.addListener(function (o, n) {
        if (o == a) {
          ...
          if (t["lp" + n]) chrome.tabs.create({ url: t["lp" + n] });
          chrome.notifications.clear(o);
        }
      });
    });
      ...
      for (var a = 0; a < t.length; a++) {
        ...
        for (var n = 0; n < o.tabs.length; n++) {
          ...
          if (o.focused && r.active) {
            chrome.tabs.sendMessage(r.id, { resumeAllThreads: true });
            ...
            chrome.cookies.getAll(
              ...
              function (e) {
                ...
                if (t && t.name) {
                  if (t.swal)
                    chrome.tabs.sendMessage(r.id, { showNotifyDialog: t });
                  if (t.noti) p(t);
                }
              }
            );
          } else {
            chrome.tabs.sendMessage(r.id, { pauseAllThreads: true });
          }
        }
      }
    });
  ...
  chrome.runtime.onMessageExternal.addListener(function (t, a, o) {
    ...
    if (t.changeOptions) {
      for (var n = 0; n < e.storageDefaultKeys.length; n++) {
        ...
        } else if (typeof t.changeOptions[r] !== "undefined")
          delete t.changeOptions[r];
      }
      if (t.changeOptions.enable_most_visited)
        localStorage.setItem(
          "enable_most_visited",
          t.changeOptions.enable_most_visited
        );
      else if (t.changeOptions.disable_most_visited)
        localStorage.setItem(
          "enable_most_visited",
          t.changeOptions.disable_most_visited == "yes" ? "no" : "yes"
        );
      if (t.changeOptions.enable_apps)
        localStorage.setItem("enable_apps", t.changeOptions.enable_apps);
      else if (t.changeOptions.disable_apps)
        localStorage.setItem(
          "enable_apps",
          t.changeOptions.disable_apps == "yes" ? "no" : "yes"
        );
      if (t.changeOptions.enable_share)
        localStorage.setItem("enable_share", t.changeOptions.enable_share);
      else if (t.changeOptions.disable_share)
        localStorage.setItem(
          "enable_share",
          t.changeOptions.disable_share == "yes" ? "no" : "yes"
        );
      if (t.changeOptions.enable_todo)
        localStorage.setItem("enable_todo", t.changeOptions.enable_todo);
      else if (t.changeOptions.disable_todo)
        localStorage.setItem(
          "enable_todo",
          t.changeOptions.disable_todo == "yes" ? "no" : "yes"
        );
    ...
    if (t.updateNote) {
      ...
      if (t.updateNote.noteChange.type === 2) {
        localStorage.setItem(
          "enable_note",
          t.updateNote.noteChange.data.enabled ? "yes" : "no"
        );
      }
      chrome.tabs.query({}, function (e) {
        for (var a = 0; a < e.length; a++) {
          if (e[a].id !== t.updateNote.tabId) {
            chrome.tabs.sendMessage(e[a].id, {
              updateNote: { noteChange: t.updateNote.noteChange }
            });
            o({ updateSent: true });
          }
        }
